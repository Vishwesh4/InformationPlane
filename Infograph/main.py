...
from evaluate_embedding import evaluate_embedding, val_logistic_classify
from gin import GNN_Infograph
...
class InfoGraph(nn.Module):
  def __init__(self, hidden_dim, num_gc_layers, alpha=0.5, beta=1., gamma=.1):
    ...
    self.encoder = GNN_Infograph(in_channel=7, hidden_channel=32,out_channel=2,num_gc_layers=3,batch_norm=True)
...
if __name__ == '__main__':
    ...
    model.eval()
    emb, y, IB_info = model.encoder.get_embeddings(dataloader)
    ...
    with open('../Results/mi_gnn_infomax_mutag.npy', 'wb') as f:
        np.save(f, XH)
        np.save(f, YH)
