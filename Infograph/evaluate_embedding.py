...
def val_logistic_classify(x_train,y_train,x_test,y_test, device):
    """
    For testing the representation of model
    Trains on x_train and y_train and predicts on x_test
    Gives output in terms on y_pred_train, y_pred_test
    """
    ...
    hid_units = x_train.shape[1]

    train_embs, test_embs = x_train.to(device), x_test.to(device)
    train_lbls, test_lbls= y_train.to(device), y_test.to(device)
